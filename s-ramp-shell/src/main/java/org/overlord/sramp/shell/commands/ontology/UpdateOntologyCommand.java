...
public class UpdateOntologyCommand extends BuiltInShellCommand {
	...
	public boolean execute() throws Exception {
        String ontologyIdArg = this.requiredArgument(0, messages.format("UpdateOntology.InvalidArgMsg.OntologyId")); //$NON-NLS-1$
        String filePathArg = this.requiredArgument(1, messages.format("UpdateOntology.InvalidArgMsg.MissingPath")); //$NON-NLS-1$
		...
		try {
		    ...
		    if (file.exists()) {
		        ...
		        if (url != null) {
                    print(messages.format("UpdateOntology.ReadingOntology", url.toExternalForm())); //$NON-NLS-1$
		            content = url.openStream();
		        } else {
                    print(messages.format("UpdateOntology.CannotFind", filePathArg)); //$NON-NLS-1$
		        }
		    }
	        client.updateOntology(ontologyUuid, content);
            print(messages.format("UpdateOntology.SuccessfulUpdate")); //$NON-NLS-1$
		} catch (Exception e) {
            print(messages.format("UpdateOntology.UpdateFailed")); //$NON-NLS-1$
