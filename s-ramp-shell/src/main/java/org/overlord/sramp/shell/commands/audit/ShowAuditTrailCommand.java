...
public class ShowAuditTrailCommand extends BuiltInShellCommand {
	...
	public boolean execute() throws Exception {
        String artifactIdArg = this.requiredArgument(0, messages.format("AuditTrail.InvalidArgMsg.ArtifactId")); //$NON-NLS-1$
        ...
        print(messages.format("ShowAuditTrail.EntriesSummary", auditTrail.size())); //$NON-NLS-1$
        print("  Idx  " + messages.format("AuditEntryLabel")); //$NON-NLS-1$ //$NON-NLS-2$
        print("  ---  -----------"); //$NON-NLS-1$
        int idx = 1;
