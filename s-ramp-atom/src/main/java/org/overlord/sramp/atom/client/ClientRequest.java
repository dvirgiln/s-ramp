...
public class ClientRequest extends org.jboss.resteasy.client.ClientRequest {
	...
	private <T> void handlePotentialServerError(ClientResponse<T> response) throws Exception {
		...
		if (response.getStatus() == 500) {
            Exception error = new Exception(messages.format("UNKNOWN_SRAMP_ERROR")); //$NON-NLS-1$
		...
		if (response.getStatus() == 404 || response.getStatus() == 415) {
            SrampAtomException error = new SrampAtomException(messages.format("ENDPOINT_NOT_FOUND")); //$NON-NLS-1$
			throw error;
		}
		if (response.getStatus() == 403) {
            SrampAtomException error = new SrampAtomException(messages.format("AUTHORIZATION_FAILED")); //$NON-NLS-1$
			throw error;
		}
		if (response.getStatus() == 401) {
            SrampAtomException error = new SrampAtomException(messages.format("AUTHENTICATION_FAILED")); //$NON-NLS-1$
            throw error;
		}
	}
